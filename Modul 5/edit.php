...
if(isset($_GET['nim'])) {
    ...
    if(isset($_POST['submit'])){
        ...
        if($kelas == "A" OR $kelas =="B" OR $kelas == "C") {
            ...
            if(mysqli_num_rows($sql) == 0) {
                ...
                $query=mysqli_query($dbc, "UPDATE `mahasiswa` SET `Nama`='$nama',`Kelas`='$kelas',`Alamat`='$alamat' WHERE NIM='$nim'");
                if($query) {
    ...
    $querya=mysqli_query($dbc,"SELECT * FROM mahasiswa WHERE NIM='$nim'");
